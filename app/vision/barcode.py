"""Barcode detection on a captured product image.

Runs pyzbar (zbar backend) over a captured frame to decode 1D/2D barcodes.
Used first in the item-routing pipeline: a decodable barcode means packaged
goods (see product_lookup.py); no barcode means the produce flow. Designed
to run fully offline on the UNO Q's MPU.
"""
